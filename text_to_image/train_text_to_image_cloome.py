...
from rdkit.DataStructs import ConvertToNumpyArray  # Import DataStructs
...
### CLOOME TOKENIZER (SMILES to Morgan)
...
class CLOOMETokenizer(PreTrainedTokenizer):
    ...
    def batch_decode(self, tokens):
        # Implement if necessary for your pipeline
        pass
...
### DOWNLOAD CLOOME CHECKPOINT AND LOAD IT
...
def load(model_path, device, model, image_resolution):
    ...
    # Create configuration from the loaded info
...
def calculate_sampling_weights(dataset, caption_column):
    print("Calculating sampling weights !")
    print("Dataset column names :")
    print(dataset.column_names)
    print("Dataset Type : ", type(dataset))
    captions = dataset[caption_column]
    frequencies = Counter(captions)

    total_samples = len(captions)

    sampling_weights = {caption: total_samples / freq for caption, freq in frequencies.items()}

    weights = [sampling_weights[caption] for caption in captions]
...
def main():
    ...
    tokenizer = CLOOMETokenizer(radius=3, nBits=1024)
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        text_encoder, preprocess = load(checkpoint_path, accelerator.device, "RN50", 520)
    ...
    if args.dataset_name is not None:
        ...
        # print("TRAIN DIR : ", args.train_data_dir)
        data_files = {}
    ...
    # text_dataset = train_datasetk.remove_columns(['image']).map(lambda x: x, remove_columns=['image'])
    print("ISOLATING TEXT ! ")
    sampling_dataset = dataset["train"]
    sampling_weights = calculate_sampling_weights(sampling_dataset, "text")
    print("Sampling !")
    sampler = WeightedRandomSampler(weights=sampling_weights, num_samples=len(sampling_weights), replacement=True)

    # DataLoaders creation:
    print("Loading in with sampler !")
    ...
    print("GLOBAL STEP : ", initial_global_step)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # GETTING RID OF RETURN_DICT
                # encoder_hidden_states = text_encoder(batch["input_ids"], return_dict=False)[0]
                encoder_hidden_states = text_encoder.encode_text(text=batch["input_ids"])
                ...
                # print("PADDED ENCODER HIDDEN STATES SHAPE : ", encoder_hidden_states.shape)
